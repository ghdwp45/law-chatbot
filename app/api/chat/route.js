...
const TOOL_DEFS = [
  ...
  },
  {
    ...
    description: '판례·해석례·조세심판·국세청해석·헌재 등 통합검색. domain으로 도메인을 지정한다.',
    input_schema: {
      type: 'object',
      properties: {
        domain: {
        ...
        court: { type: 'string', description: '법원명 (domain=precedent 시 선택)' },
        caseNumber: { type: 'string', description: '사건번호 (domain=precedent 시 선택)' },
        fromDate: { type: 'string', description: '시작일 YYYYMMDD' },
        toDate: { type: 'string', description: '종료일 YYYYMMDD' },
        cls: { type: 'string', description: '재결구분코드 (domain=tax_tribunal 시 선택)' },
        dpaYd: { type: 'string', description: '처분일 범위 YYYYMMDD~YYYYMMDD (domain=tax_tribunal 시 선택)' },
        rslYd: { type: 'string', description: '결정일 범위 YYYYMMDD~YYYYMMDD (domain=tax_tribunal 시 선택)' },
      },
...
- 판례·해석례·심판례가 필요하면 search_decisions를 domain별로 병렬 호출한다:
  · domain=precedent (법원 판례)
  · domain=interpretation (기재부·국세청 법령해석·예규)
  · domain=tax_tribunal (조세심판원 결정례)
...
- 도구 조회에 모두 실패한 경우, 답변 최상단에 반드시 아래 배너를 표시한다:
  ▎ [주의: 실시간 법령 조회 실패 — 아래 내용은 AI 내장 지식 기반이며 최신 개정사항이 반영되지 않을 수 있습니다]
...
function toToolResultText(r) {
  ...
  if (r && r.isError) {
    const errText = Array.isArray(r.content)
      ? r.content.map((p) => p?.text ?? JSON.stringify(p)).join('\n')
      : JSON.stringify(r);
    return `조회 실패(서버): ${errText}`;
  }
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        const safeMsg = e.message.replace(process.env.LAW_OC ?? '', '***');
        console.error('[ERROR] MCP 연결 실패:', safeMsg);
        send({ error: '법령 서버 연결 실패. 잠시 후 다시 시도해 주세요.' });
        controller.close();
        return;
      }
      ...
      // 스텝 소진 시 수집한 근거로 도구 없이 최종 답변 합성
        ...
        for (let step = 0; step < MAX_STEPS; step++) {
          ...
          for await (const ev of ms) {
            if (ev.type === 'content_block_start' && ev.content_block?.type === 'tool_use') {
              console.log('[DEBUG] 도구 호출:', ev.content_block.name);
          ...
          // 도구 호출 없으면 최종 답변
          if (toolUses.length === 0) return stepText;

          convo.push({ role: 'assistant', content: final.content });
          const results = await Promise.all(
            toolUses.map(async (tu) => {
              const r = await runTool(tu.name, tu.input);
              const text = toToolResultText(r);
              toolCallLog.push({ name: tu.name, success: !r?.__error && !r?.isError });
      ...
      try {
        ...
        console.log('[INFO] 요약:', JSON.stringify({
          calls: toolCallLog.map((c) => `${c.name}:${c.success ? 'O' : 'X'}`),
          truncated: truncatedOut,
        }));

        send({ text: finalText });
        send({ done: true, full: finalText });
      } catch (e) {
        const msg =
