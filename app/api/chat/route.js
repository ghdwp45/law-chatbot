...
async function runNtsSearch({ query, tlaw, top_k }) {
  try {
    ...
    const body = merged
      ...
      .map((r, i) =>
        `[결과 ${i + 1}] ${r.title}\n` +
        `문서ID: ${r.doc_id} | 세목: ${r.tlaw} | 생산일자: ${r.prod_date}\n` +
...
- 국세청 해석례(nts)는 전문(원문) 조회가 지원되지 않는다. 따라서 nts 해석례를 인용할 때는 search_decisions 결과에 함께 제공된 '링크:' URL을 답변에 그대로 명시해 사용자가 원문을 직접 확인할 수 있게 한다. 이 링크는 도구 결과에 실제로 표시된 것만 사용하고, URL을 임의로 만들거나 추측해 적지 말 것. 결과에 링크가 없으면 제목·문서번호만 제시하고 '원문 미확인'임을 밝힌다.
- 도구 결과가 질문과 무관하면 억지로 엮지 말고 무관함을 밝힌다.
- 구체적 사실관계에 대한 과세관청의 실무 해석이 필요하면(예: "이런 경우 세금계산서 발급 시기는?") search_nts_taxlaw로 국세청 질의회신 사례를 함께 조회한다. search_decisions의 nts 도메인과는 다른 별도의 더 큰 데이터베이스이므로, 법령·예규만으로 결론이 막연하면 이걸로 보강한다. tlaw(세목)를 지정하면 정확도가 올라간다.
...
[출처 태그 - 필수]
📋 [법령 원문] / ⚖️ [판례·해석례](사건번호·예규번호 명시) / 🗂️ [국세청 질의회신](문서ID·생산일자·세목 명시) / 📘 [K-IFRS](기준서번호·문단 또는 QnA 일자 명시) / 💡 [AI 해설] / ⚠️ [AI 추정]
- 별표(★)·신뢰도 점수 표시 금지.
