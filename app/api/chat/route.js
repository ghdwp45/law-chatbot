...
[해석쟁점 필수 조회]
- 적법성·허용 여부·공급시기·귀속시기·과세대상 여부처럼 법령 문언만으로 단정하기 어려운 해석 쟁점은, 반드시 search_decisions로 기재부 예규(interpretation)·국세청 해석(nts)·조세심판원(tax_tribunal)·법원 판례(precedent)를 조회하고 get_decision_text로 전문을 확인한 뒤 결론을 내린다. 법령 조문만 읽고 적법/위법을 단정하지 말 것.
...
export async function POST(req) {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      const mcpClient = new Client({ name: 'law-chatbot', version: '3.0.0' }, { capabilities: {} });
      ...
      async function generate(maxSteps) {
        for (let step = 0; step < maxSteps; step++) {
          ...
          if (toolUses.length === 0) return { answerText: stepText, incomplete: false };
          if (step === maxSteps - 1) return { answerText: INCOMPLETE_ANSWER, incomplete: true };
        ...
        return { answerText: INCOMPLETE_ANSWER, incomplete: true };
      }
...
3. 무근거 인용: 답변의 사건번호·예규번호·핵심 사실이 [확보된 근거]에 존재하지 않는가.
...
판정: 문제 없으면 action="pass". 추가 조회·수정으로 고칠 수 있으면 "revise"(대부분). 근거를 댈 수 없는 위험한 단정이면 "block". instructions에는 무엇을 어떻게 고칠지 한국어로 구체적으로 적는다.`;
      ...
      try {
        let { answerText, incomplete } = await generate(MAX_STEPS);

        // LLM-judge 검증 + 1회 재작성 (미완료 답변은 제외)
        if (!incomplete) {
          const judge = await runJudge(lastUserMsg, answerText);
          const det = evaluateIntegrity(answerText, stats);
          const needRewrite = judge.action === 'revise' || judge.action === 'block' || det.fatal.length > 0;

          if (!IS_PROD) {
            console.log('[DEBUG] judge:', JSON.stringify({ action: judge.action, reasons: judge.reasons, fatal: det.fatal }));
          }

          if (needRewrite) {
            send({ revising: true });
            const reasons = [...new Set([...judge.reasons, ...det.fatal])];
            const reviseMsg =
              `직전 답변에 다음 문제가 있어 재작성이 필요합니다:\n- ${reasons.join('\n- ')}\n\n` +
              `${judge.instructions ? judge.instructions + '\n' : ''}` +
              `${judge.requireDecisionLookup ? '특히 적법성·공급시기 등 해석 쟁점이므로 search_decisions(interpretation·nts·tax_tribunal·precedent)와 get_decision_text로 관련 예규·심판례를 반드시 조회한 뒤, 전문으로 확인한 사건·예규번호만 인용하라.\n' : ''}` +
            ...
            if (!second.incomplete && second.answerText) answerText = second.answerText;
          }
        }

        // 최종 결정적 게이트 (재작성 후에도 잔존하는 결함 차단)
        if (answerText !== INCOMPLETE_ANSWER) {
          ...
          if (!IS_PROD && (fatal.length || warnings.length)) {
            console.log('[DEBUG] final integrity:', JSON.stringify({ fatal, warnings }));
          }
          if (fatal.length) {
